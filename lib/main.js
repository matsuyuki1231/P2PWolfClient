...
function adjustmentMemberVolumeSliderBackGround(element) {
    const volumePercentage = element.value / element.max * 100;
    element.style.background = "linear-gradient(to right, rgba(94, 0, 255, 0.32) " + volumePercentage + "%, rgba(255, 255, 255, 0) " + volumePercentage + "%)"
}
...
function establishWebSocketConnection() {
    ...
    try {
        webSocket = new WebSocket("wss://vc-ymgs.f5.si:8291");
    } catch (exception) {
    ...
    webSocket.onclose = event => {
        ...
        if (connectionFailedCount++ === 5) {
            noticeLog("インターネット接続を確認いただき、問題がない場合はよもぎサーバー運営にこの問題を報告してください")
    ...
    webSocket.onmessage = event => {
        ...
        switch (data["type"]) {
            case "show_user_info":
                ...
                me.addElements();
                Promise.allSettled([getMikeStreamPromise, connectionConfirmButtonClickedPromise]).then(() => webSocket.send(JSON.stringify({"type":"join_request"})));
                break;
            ...
            case "remote_sound": {
                ...
                (new Promise(async () => {
                    ...
                    const gainNode = audioContext.createGain();
                    gainNode.gain.value = 0.3;
                    audioBufferSourceNode.onended = () => SoundEffects.getInstance().play("chime");
