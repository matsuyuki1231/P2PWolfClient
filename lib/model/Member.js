class Member {
    ...
    resetGainNode() {
        ...
        this.gainNode.gain.value = this.getDistanceVolume() * (!UserSetting.getInstance().isDeaf()) * (!this.isServerMuted) * (!this.isMute) * (this.getVolume() / 100);
    }
    ...
    addElements() {
        ...
        if (this.isSelf) {
            const virtualVolumeRange = document.createElement("div");
            ...
            volumeRange.min = "0";
            volumeRange.max = "100";
            volumeRange.value = this.getVolume().toString();
            adjustmentMemberVolumeSliderBackGround(volumeRange);
            volumeRange.addEventListener("input", event => {
                adjustmentMemberVolumeSliderBackGround(event.target);
                this.setVolume(event.target.value);
            })
            volumeAdjustableBox.appendChild(volumeRange);
        }
    ...
    setSignal() {
        ...
        this.peer.on("stream", stream => {
            ...
            this.gainNode.gain.value = this.getDistanceVolume() * (!UserSetting.getInstance().isDeaf()) * (!this.isServerMuted) * (!this.isMute) * (this.getVolume() / 0.7);
            this.stereoPannerNode.pan.value = this.getPan();
            streamMediaSource.connect(this.gainNode).connect(this.stereoPannerNode).connect(destination);
